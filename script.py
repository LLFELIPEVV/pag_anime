    ...
    def handle(self, *args, **kwargs):
        ...
        for anime in anime_list:
            try:
                ...
                if not info_anime:
                    continue  # Skip this anime if there's no information
                ...
                nuevo_anime, _ = Anime.objects.get_or_create(
                    id=id_anime,
                    defaults={
                        'titulo': title,
                        'poster_url': poster,
                ...
                for genre in genres:
                    ...
                    nuevo_genero, _ = Generos.objects.get_or_create(nombre_genero=genre)
                ...
                for episode in episodes:
                    episodios_id = episode.id
                    episodio_anime = nuevo_anime
                    episode_number = episode.id
                    episode_preview = episode.image_preview
                    
                    print(f"Insertando episodio con numero: {episode.id}")
                    nuevo_episodio, _ = Episodios.objects.get_or_create(
                        id=episodios_id,
                        defaults={
                    ...
                    download_links = api.get_links(f"{id_anime}", f"{episodios_id}")
                    for link in download_links:
                        download_server = link.server
                        download_url = link.url

                        nuevo_download, _ = Download_Server.objects.get_or_create(
                    ...
                    # Tabla Video_server
                    video_servers = api.get_video_servers(f"{id_anime}", f"{episodios_id}")
                    for server in video_servers[0]:
                        ...
                        nuevo_video, _ = Video_server.objects.get_or_create(
                            episodio_id=nuevo_episodio,
                            server=video_server,
                            title=video_title,
                            ads=video_ads,
                ...
                continue  # Continue with the next anime
