...
def removeprefix(str: str, prefix: str) -> str:
    ...
    prefix for compatibility with Python >3.9

    :param str: string to remove prefix from.
    :param prefix: prefix to remove from the string.
    :rtype: str
    """
...
def parse_table(table: Tag):
    ...
    # Verificar si 'thead' existe en la tabla
    if table is not None:
        thead = table.find('thead')
        if thead:
            columns = [x.string for x in thead.tr.find_all("th")]

        for row in table.tbody.find_all("tr"):
            values = row.find_all("td")

            # Si no hay columnas en 'thead', intenta obtener los nombres de columna de 'td'
            if not columns:
                columns = [x.string for x in values]

            # Verificar si el número de valores coincide con el número de columnas
            if len(values) != len(columns):
                print("Warning: Row does not match columns size, skipping row.")
...
class AnimeFLV(object):
    ...
    def get_links(
        ...
        **kwargs,
        ...
        """
        Get download links of a specific episode.
    ...
    def get_latest_episodes(self) -> List[EpisodeInfo]:
        ...
        while attempt <= max_attempts:
            ...
            # If the elements are not found, wait and try again
            time.sleep(5)  # Wait for 5 seconds before trying again
            attempt += 1

        if attempt > max_attempts:
            raise AnimeFLVParseError("Unable to get list of episodes after several attempts")
    ...
    def get_latest_animes(self) -> List[AnimeInfo]:
        ...
        if attempt > max_attempts:
            raise AnimeFLVParseError("Unable to get list of animes after several attempts")
    ...
    def get_anime_info(self, id: str) -> AnimeInfo:
        ...
        :param id: Anime id, like 'yamato-takeru-after-war'.
        :rtype: AnimeInfo
        """
        ...
        while attempt <= max_attempts:
            ...
            if soup.select_one(
                ...
                break  # Page loaded successfully, exit the loop
            # If the page hasn't loaded, wait a bit before trying again
            time.sleep(5)  # Wait for 5 seconds before trying again
            attempt += 1
            
        if attempt > max_attempts:
            raise AnimeFLVParseError("Unable to load the page after several attempts.")
        ...
        information = {
            ...
            "synopsis": (
                soup.select_one("body div div div div div main section div.Description p").string.strip()
                if soup.select_one("body div div div div div main section div.Description p") is not None and soup.select_one("body div div div div div main section div.Description p").string is not None
                else ""  # Use an empty string if synopsis is not available
            ),
        ...
        try:
            for script in soup.find_all("script"):
                ...
                if "var anime_info = [" in contents:
                    anime_info_match = re.search(r'var anime_info = \[(.*?)\];', contents)
                    if anime_info_match:
                        anime_info = anime_info_match.group(1)

                        # Replace single quote with Unicode escape sequence
                        anime_info = anime_info.replace("'", "\u0027")

                        # Now you can load it as JSON
                        info_ids.append(json.loads(f"[{anime_info}]"))
